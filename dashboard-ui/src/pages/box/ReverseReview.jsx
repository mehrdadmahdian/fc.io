    ...
    function ReverseReview() {
        ...
        useEffect(() => {
            const fetchReviewData = async () => {
                try {
                    const response = await api.get(`/dashboard/boxes/${boxId}/review/reverse/cards`);
                    if (response.data.status === 'success') {
                        ...
                        setError('Failed to fetch reverse review data');
                    }
                } catch (err) {
        ...
        const handleResponse = async (difficulty) => {
            try {
                const response = await api.post(`/dashboard/boxes/${boxId}/review/reverse/respond`, {
                ...
                if (currentCard < totalCards - 1) {
                    ...
                    alert(`Reverse review completed! You've reviewed ${currentCard + 1} cards.`);
                    navigate('/');
                }
            } catch (err) {
                setError(err.message);
                            ...
                            <div className="dashboard-box">
                                <div className="review-content empty">
                                    <div className="empty-state">
                                        ...
                                        <p>{t('review.noCards.messageReverse')}</p>
                                        <button 
