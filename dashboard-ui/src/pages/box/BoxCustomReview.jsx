...
function BoxCustomReview() {
    ...
    const fetchBoxInfo = async () => {
        try {
            ...
            console.error('Failed to fetch box info:', err);
        }
    };
    ...
    const fetchCustomReviewData = async () => {
        try {
            ...
            const response = await api.post('/dashboard/review/custom', requestData);
            ...
            if (response.data.status === 'success') {
                ...
                if (cards.length === 0) {
                    setError('No cards match your current filters. Try adjusting your selection criteria.');
                }
            } else {
                setError('Failed to fetch custom review data');
            }
        } catch (err) {
            setError(err.message || 'Failed to fetch custom review data');
        } finally {
            setLoading(false);
        }
    };
            ...
            <ReviewContainer>
                <div className="dashboard-container">
                    <div className="error-state">
                        ...
                        <p>No cards match your current filters. Try adjusting your selection criteria.</p>
                        <button 
                            className="btn btn-primary" 
                            onClick={handleBackToFilters}
                        >
...
export default BoxCustomReview;
